...
export const reduceHearts = async ( challendeId: number) => {
    ...
    const challenge = await db.query.challenges.findFirst({
        where: eq(challenges.id, challendeId)
    });
    ...
    .findFirst({
        where: and(
            ...
            eq(challengeProgress.challengeId, challendeId)
        )
    });
...
export const upsertUserProgress = async (courseId: number) => {
    ...
    if(!course.units.length || !course.units[0].lesson.length) throw new Error('Course is empty')
    ...
    if (existingUserProgress) {
        ...
        // now it's needed to clean the cache
